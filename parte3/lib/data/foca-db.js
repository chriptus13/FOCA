...
module.exports = class FocaDB {
    constructor(es) {
        ...
        this.refreshUrl = `http://${es.host}:${es.port}/${es.groups_index.split('/')[0]}/_refresh`
    }
    ...
    createGroup(userId, name, description) {
        ...
        return rp.post(uri)
            ...
            .then(data => rp.post(this.refreshUrl).then(() => data._id))
    }
     ...
     */
    getGroups(userId) {
        const uri = makeUri(`${this.url}/_search?q=userId:${userId}`)
        return rp.get(uri)
            .catch(() => Promise.reject({ message: 'FOCA DB inaccessible.', statusCode: 503 }))
            .then(data => data.hits.hits.map(grp => {
                const src = grp._source
                src.id = grp._id
                return src
            }))
    }
